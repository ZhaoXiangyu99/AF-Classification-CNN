...
class Logger(object):
    ...
    def __init__(self,
                ...
                experiment_path: str = "",
                 ...
                 path_models: str = "models",
                 is_binary_classifier: bool = False,
                 model_type: str = "CNN") -> None:
        """
        Constructor method
        ...
        if is_binary_classifier:
            folder = folder + "/two_class/"
        else:
            folder = folder + "/four_class/"
        # 区分CNN和AttNet
        if model_type == "CNN":
            folder = folder + "CNN/"
        else:
            folder = folder + "AttNet/"

        folder = folder + datetime.now().strftime("%Y_%m_%d") + "/"
        experiment_path = os.path.join(os.getcwd(), folder)
