...
parser.add_argument("--fs", default=300, type=int,help="sampling frequency")
...
from util import load_references
from config import *
...
if __name__ == '__main__':
    ...
    if args.network_config == "ECGCNN_S":
        ...
        data_logger = Logger(experiment_path_extension="ECGCNN_S" + dataset_info, model_type="CNN")
        print("ECGCNN_S utilized")
    elif args.network_config == "ECGCNN_M":
        config = ECGCNN_CONFIG_M
        data_logger = Logger(experiment_path_extension="ECGCNN_M" + dataset_info, model_type="CNN")
        print("ECGCNN_M utilized")
    elif args.network_config == "ECGCNN_L":
        config = ECGCNN_CONFIG_L
        data_logger = Logger(experiment_path_extension="ECGCNN_L" + dataset_info, model_type="CNN")
        print("ECGCNN_L utilized")
    elif args.network_config == "ECGCNN_XL":
        config = ECGCNN_CONFIG_XL
        data_logger = Logger(experiment_path_extension="ECGCNN_XL" + dataset_info, model_type="CNN")
        print("ECGCNN_XL utilized")
    elif args.network_config == "ECGAttNet_S":
        config = ECGAttNet_CONFIG_S
        data_logger = Logger(experiment_path_extension="ECGAttNet_S" + dataset_info, model_type="AttNet")
        print("ECGAttNet_S utilized")
    elif args.network_config == "ECGAttNet_M":
        config = ECGAttNet_CONFIG_M
        data_logger = Logger(experiment_path_extension="ECGAttNet_M" + dataset_info, model_type="AttNet")
        print("ECGAttNet_M utilized")
    elif args.network_config == "ECGAttNet_L":
        config = ECGAttNet_CONFIG_L
        data_logger = Logger(experiment_path_extension="ECGAttNet_L" + dataset_info, model_type="AttNet")
        print("ECGAttNet_L utilized")
    elif args.network_config == "ECGAttNet_XL":
        config = ECGAttNet_CONFIG_XL
        data_logger = Logger(experiment_path_extension="ECGAttNet_XL" + dataset_info, model_type="AttNet")
        print("ECGAttNet_XL utilized")
    elif args.network_config == "ECGAttNet_XXL":
        config = ECGAttNet_CONFIG_XXL
        data_logger = Logger(experiment_path_extension="ECGAttNet_XXL" + dataset_info, model_type="AttNet")
        print("ECGAttNet_XXL utilized")
    else:
        config = ECGAttNet_CONFIG_130M
        data_logger = Logger(experiment_path_extension="ECGAttNet_130M" + dataset_info, model_type="AttNet")
    ...
    if args.icentia11k:
        training_dataset = DataLoader(
            ...
            batch_size=max(1, args.batch_size // max_batch_size), num_workers=min(max(args.batch_size // max_batch_size, 4), 20),
        ...
        ecg_leads, ecg_labels, ecg_names = load_references(args.dataset_path)
        fs = args.fs
        if args.physio_net:
            if args.challange:
